 ...
 * `container.innerHTML = renderMiloSvg(size)` followed by `startMilos()` from `./animate`,
 * with no framework required. The Astro component in this folder is one such thin shell.
 */
...
// One continuous outline around head and ears, used as the clip
// every fur layer renders inside. The sides read like the reference:
// round cheeks, a slight temple waist, then a flare to the ears.
...
// The skull outline alone: the same head but with smooth shoulders across
// the ear bases. The coat clips against this so splotches end at the top
// of the head instead of spilling into the gray of the ear cones.
const skullPath =
...
// Whisker curves as [root, control1, control2, tip], rendered as filled slivers
// that taper from the root width to a point at the tip, like real hairs.
...
>
  ...
  <g data-milo-head>
    <g clip-path="url(#${uid}-face-clip)">
      ...
      <!-- Face plane: every marking painted on the face rides this group, which the
      script slides with the features (at a fraction of their travel) so the pattern
      turns with the head instead of staying glued to the skull. Shapes bleed past
      the silhouette so sliding never exposes a seam, and the patches near the skull
      dome sit low enough that a full deflection never shears them against it. -->
      <g clip-path="url(#${uid}-skull-clip)">
      <g data-milo-face>
        <!-- The coat, traced directly from the reference sticker: each subpath is a patch
        contour extracted from the image (lighter gray, cream, then dark gray over the
        mid-gray base), so placement, size, angle, and raggedness are the source's own. -->
        ...
        <!-- White blaze and muzzle: a narrow strip between the eyes that flares
        around the muzzle, extended past the chin so it stays seamless in motion. -->
        <path
      ...
      moving plane. Beneath them lies only the uniform crown gray (the splotch zone ends
      below their bottoms), so their slower slide can never shear a marking, and their small
      travel stays inside the cone frame, so the shift reads as the ears foreshortening. -->
      <g data-milo-ears>
        <path
  ...
  <g data-milo-features>
    ...
    <!-- Tan surrounds hugging each eye like the reference's goggle markings. They
    ride the features group so they stay glued to the eyes at any deflection. -->
    <path
    ...
    <!-- Eyes: wide almonds with an ~19-degree corner slant baked into the paths, inner corners
    dipping toward the blaze. Proportions started from measuring the reference (19x15 against
    a 118-unit head, centers 40 apart), then the whole construction was scaled up ~30% for
    presence with the measured inner-corner gap preserved. The pupil group slides further than
    the eye for a "looking at you" read. Highlights sit on the pupil edge, asymmetric per eye. -->
    ...
    <!-- Nose: a subtle off-white bridge above it, then the pink pad (sheen, thin
    nostril slits, dark tip), the mouth notch beneath, and soft muzzle mounds,
    giving a neutral cat mouth like the reference rather than a smile. -->
    <path
